...
@section('title', 'Lupa Kata Sandi - e-Rapor')
...
<div class="min-h-screen bg-[#F4F6F9] flex flex-col relative font-sans selection:bg-[#003399]/25 selection:text-[#003399]">
    ...
    <div class="relative bg-[#003399] pt-10 pb-20 overflow-hidden z-0">
        ...
        <div class="px-6 relative z-20 mt-8 max-w-md mx-auto">
            ...
            <p class="text-[11px] text-white/70 max-w-[280px] font-normal leading-relaxed">
                Lupa kata sandi? Masukkan alamat email Anda dan kami akan mengirimkan tautan pemulihan kata sandi.
            </p>
        </div>
    ...
    <div class="flex-1 px-6 pt-8 w-full max-w-md mx-auto relative z-10 pb-28 lg:pb-12">
        ...
        <form method="POST" action="{{ route('password.email') }}" id="resetPasswordForm" class="flex flex-col">
            @csrf
            ...
            <div class="mb-8 relative group">
                ...
                <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="Masukkan email terdaftar Anda"
    ...
    <div class="lg:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 p-4 pb-6 shadow-[0_-10px_20px_-10px_rgba(0,0,0,0.05)] z-50">
        <div class="max-w-md mx-auto">
            <button type="submit" form="resetPasswordForm" class="w-full bg-[#003399] hover:bg-[#002266] text-white font-black text-xs uppercase tracking-widest py-4 rounded-xl transition-all shadow-lg shadow-[#003399]/20 flex items-center justify-center gap-2 border-b-2 border-slate-900/20">
                <i class="fa-solid fa-paper-plane text-[#FFB800]"></i>
                Kirim Tautan Reset
            </button>
        </div>
