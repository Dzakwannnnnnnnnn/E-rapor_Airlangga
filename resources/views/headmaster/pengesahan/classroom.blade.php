...
@section('title', 'Pengesahan Kelas ' . $classroom->name . ' · e-Rapor')
@section('back_url', route('headmaster.pengesahan.index'))
@section('content')
    ...
    <div class="bg-primary text-white pt-10 pb-20 px-6 md:px-8 relative overflow-hidden select-none">
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
            ...
            <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                ...
                <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs text-white/85 font-medium">
                    <div class="flex items-center gap-2">
                        <i class="fa-solid fa-chalkboard-user text-secondary"></i>
                        <span>Wali Kelas: <strong>{{ $classroom->homeroomTeacher?->user?->name ?? 'Belum Ditentukan' }}</strong></span>
                    </div>
                    <div class="flex items-center gap-2">
                        <i class="fa-solid fa-calendar text-secondary"></i>
                        <span>{{ ucfirst($activeYear->semester) }} {{ $activeYear->year }}</span>
                    </div>
                </div>
            </div>
        </div>
...
<div class="max-w-6xl mx-auto mt-6 space-y-6 px-4 md:px-0 pb-16">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="px-5 py-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            {{-- Sahkan Semua --}}
            @php
                $hasPendingInClass = $reportCards->filter(fn($rc) => $rc->is_submitted && !$rc->is_validated)->count();
            @endphp
            @if($hasPendingInClass > 0)
            <form action="{{ route('headmaster.pengesahan.validate-all', $classroom->id) }}" method="POST" class="inline"
                  onsubmit="return confirm('Sahkan {{ $hasPendingInClass }} rapor yang sudah diajukan di kelas {{ $classroom->name }}? Tindakan ini akan langsung mengizinkan orang tua mengakses rapor.')">
                @csrf
                <button type="submit"
                        class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black uppercase tracking-wider transition-all shadow-md cursor-pointer flex items-center justify-center gap-2">
                    <i class="fa-solid fa-stamp"></i>
                    Sahkan Semua ({{ $hasPendingInClass }})
                </button>
            </form>
            @else
            <div class="flex items-center gap-2 text-[11px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-3 py-2 rounded-xl">
                <i class="fa-solid fa-circle-check"></i>
                Tidak ada rapor yang menunggu
            </div>
            @endif
        </div>
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="text-right px-5 py-3 text-[10px] font-black text-slate-500 uppercase tracking-widest w-[220px]">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    ...
                    @endphp
                    <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-slate-50/20' }} {{ $rc && $rc->is_submitted && !$rc->is_validated ? 'ring-1 ring-inset ring-blue-100' : '' }} hover:bg-blue-50/10 transition-colors">
                        ...
                        <td class="px-4 py-4 text-xs text-slate-500 font-medium leading-relaxed italic">
                            {{ $rc ? ($rc->description ?: 'Tidak ada catatan.') : '–' }}
                        </td>

                        <td class="px-4 py-4 text-center">
                            <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full border text-[9px] font-black uppercase {{ $statusStyle }}">
                                {{ $statusText }}
                            </span>
                        </td>
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                @if($rc && $rc->is_submitted && !$rc->is_validated)
                                    {{-- Sahkan --}}
                                    <form action="{{ route('headmaster.pengesahan.validate', $rc->id) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Sahkan rapor {{ $student->name }}? Orang tua akan langsung bisa mengaksesnya.')">
                                        @csrf
                                        <button type="submit" class="px-2.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold uppercase transition shadow-sm cursor-pointer">
                                            <i class="fa-solid fa-stamp mr-1"></i> Sahkan
                                        </button>
                                    </form>

                                    {{-- Tolak --}}
                                    <form action="{{ route('headmaster.pengesahan.reject', $rc->id) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Tolak dan kembalikan rapor {{ $student->name }} ke Wali Kelas?')">
                                        @csrf
                                        <button type="submit" class="px-2.5 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white text-[10px] font-bold uppercase transition shadow-sm cursor-pointer">
                                ...
                                @elseif($rc && $rc->is_validated)
                                    {{-- Batalkan Sah --}}
                                    <form action="{{ route('headmaster.pengesahan.reject', $rc->id) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Batalkan pengesahan rapor {{ $student->name }}? Orang tua tidak bisa mengaksesnya lagi.')">
                                        @csrf
                                        <button type="submit" class="px-2.5 py-1.5 rounded-lg border border-rose-200 text-rose-600 bg-rose-50 hover:bg-rose-100 text-[10px] font-bold uppercase transition cursor-pointer">
