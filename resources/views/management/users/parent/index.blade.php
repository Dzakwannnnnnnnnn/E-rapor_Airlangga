...
@section('title', 'Manajemen Wali Murid - e-Rapor')
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center md:items-start justify-between gap-6 relative z-10">
            <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                ...
                <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                    <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase tracking-wide">Manajemen Wali Murid</h3>
                    <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs md:text-sm text-white/85 font-medium">
                        <div class="flex items-center gap-2">
                            ...
                            <span>Kelola Data Wali Murid / Orang Tua</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="shrink-0 mt-2 md:mt-4">
                <a href="{{ route('admin.parents.create') }}" class="inline-flex items-center gap-2 bg-secondary text-primary hover:bg-white font-black text-sm px-6 py-3 rounded-full transition-all shadow-lg active:scale-95">
                    <i class="fa-solid fa-plus"></i> Tambah Wali Murid
                </a>
            </div>
        </div>
...
<div class="max-w-6xl mx-auto mt-6 space-y-6">
    ...
    <!-- Overlapping Card: Total Wali Murid -->
    <div class="max-w-xl mx-auto -mt-16 mb-6 relative z-20 px-4">
        <div class="bg-white rounded-2xl shadow-md border border-slate-100 p-5 flex items-center gap-4">
            ...
            <div>
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Wali Murid</span>
                <span class="text-xl font-black text-slate-800 leading-none mt-1 block">{{ $parents->total() }} <span class="text-sm font-bold text-slate-500">Akun</span></span>
            </div>
        </div>
    </div>
    ...
    <div class="px-4 md:px-0 pb-12">
        ...
        <div class="mb-6 flex flex-col sm:flex-row gap-3">
            <form action="{{ request()->url() }}" method="GET" class="relative flex-1">
                ...
                <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama atau email wali murid..." class="w-full pl-11 pr-12 py-3 bg-white border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all shadow-sm">
        ...
        <div class="space-y-4">
            <div class="flex items-center gap-2 mb-2">
                <i class="fa-solid fa-users-rays text-yellow-600"></i>
                <h4 class="font-bold text-slate-800 text-sm uppercase tracking-wide">Daftar Wali Murid</h4>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @forelse ($parents as $parent)
                    @php
                        // Cek status verifikasi email / aktivasi akun wali murid
                        $isVerified = !is_null($parent->email_verified_at);
                    @endphp
                    ...
                    <div class="relative bg-white rounded-xl border border-slate-100 p-4 sm:p-5 flex flex-col justify-between gap-4 hover:shadow-md hover:border-slate-200 transition-all duration-300 group">
                        ...
                        <div class="flex items-start gap-3 sm:gap-4">
                            <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-amber-50 text-amber-700 flex items-center justify-center font-bold text-sm sm:text-base shrink-0 border border-amber-100/50 tracking-wide">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span class="text-xs text-slate-500 block truncate mt-1">
                                    <i class="fa-solid fa-envelope mr-1.5 opacity-60"></i>{{ $parent->email }}
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2 pt-0.5">
                            <span class="inline-flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-lg text-xs font-medium border border-slate-100 text-slate-600">
                                <i class="fa-solid fa-people-arrows text-slate-400 text-[10px]"></i>
                                Hubungan: <span class="font-bold text-slate-700 capitalize pl-0.5">{{ $parent->parent->relation ?? '-' }}</span>
                            </span>
                        ...
                        <div class="grid grid-cols-3 gap-2 pt-3 border-t border-dashed border-slate-100 mt-auto w-full">
                            <a href="{{ route('admin.parents.show', $parent->id) }}" class="inline-flex justify-center items-center gap-1.5 px-2 py-2 bg-slate-50 text-slate-600 hover:text-primary hover:bg-blue-50/50 rounded-lg transition-colors text-xs font-bold border border-slate-100" title="Lihat Detail">
                                <i class="fa-solid fa-eye text-xs sm:text-sm opacity-80"></i> <span class="truncate">Detail</span>
                            </a>

                            <a href="{{ route('admin.parents.edit', $parent->id) }}" class="inline-flex justify-center items-center gap-1.5 px-2 py-2 bg-slate-50 text-slate-600 hover:text-amber-600 hover:bg-amber-50/50 rounded-lg transition-colors text-xs font-bold border border-slate-100" title="Edit Data">
                            ...
                            <form
                                ...
                                data-confirm="Akun wali murid &quot;{{ $parent->name }}&quot; akan dihapus secara permanen dan tidak bisa dikembalikan."
                                data-confirm-title="Hapus Wali Murid?"
                ...
                @empty
                    <div class="md:col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 p-12 text-center flex flex-col items-center">
                        ...
                        <p class="text-sm font-bold text-slate-600">Belum ada data Wali Murid</p>
                        @if($search)
