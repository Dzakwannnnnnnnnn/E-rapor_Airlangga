...
@section('title', 'Edit Tahun Ajaran - ' . $academicYear->year . ' - e-Rapor')
        ...
        <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
            ...
            <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                <div class="inline-flex items-center gap-1.5 bg-secondary text-[10px] text-slate-950 font-bold px-2.5 py-0.5 rounded-full mb-2.5 uppercase tracking-wider shadow-sm">
                    <i class="fa-solid fa-calendar-check text-[9px]"></i> ID Tahun: #{{ $academicYear->id }}
                </div>
                <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase truncate tracking-wide">Edit Tahun Ajaran</h3>
                ...
                <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs md:text-sm text-white/85 font-medium">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span>Semester: {{ ucfirst($academicYear->semester) }}</span>
                    </div>
                </div>
            </div>
        </div>
...
<div class="max-w-4xl mx-auto mt-6 space-y-6">
    ...
    <form id="edit-academic-form" method="POST" action="{{ route('admin.academic_years.update', $academicYear->id) }}" class="space-y-6 px-4 sm:px-0">
        @csrf
        @method('PUT')

        <div class="space-y-3">
            <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Perbarui Informasi Periode</h4>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="year" id="year" value="{{ old('year', $academicYear->year) }}" required
                            class="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 focus:border-primary rounded-xl text-sm font-medium text-slate-800 shadow-sm transition-all">
                    </div>
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <select name="is_active" id="is_active" required class="w-full px-4 py-3.5 bg-slate-50/50 border border-slate-200 focus:border-primary rounded-xl text-sm font-medium text-slate-800 shadow-sm appearance-none cursor-pointer">
                            <option value="0" {{ old('is_active', $academicYear->is_active) == 0 ? 'selected' : '' }}>Tidak Aktif</option>
        ...
        <div class="bg-blue-50 border-l-4 border-primary text-slate-900 rounded-2xl p-4 shadow-sm flex items-center gap-4 select-none">
            ...
            <div class="flex-1">
                <h5 class="font-extrabold text-xs uppercase tracking-wide text-primary">Perhatian</h5>
                <p class="text-xs text-slate-600 mt-0.5 leading-normal">Mengubah status menjadi "Aktif" akan menonaktifkan tahun ajaran lainnya secara otomatis demi menjaga konsistensi data e-Rapor.</p>
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const form = document.getElementById('edit-academic-form');
...
<div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-100 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] px-4 py-3 pb-safe">
    <div class="flex items-center gap-3 max-w-lg mx-auto">
        ...
        <button type="submit" form="edit-academic-form" class="flex-[2] flex items-center justify-center gap-2 py-3 rounded-2xl bg-gradient-to-br from-primary to-blue-800 text-white font-black text-xs uppercase tracking-wider shadow-lg shadow-primary/30 hover:shadow-primary/50 transition-all border-b-2 border-blue-900">
