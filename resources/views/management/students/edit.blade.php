...
@section('title', 'Perbarui Data Siswa - e-Rapor')
    ...
    <div class="bg-primary text-white pt-10 pb-20 px-6 md:px-8 relative overflow-hidden select-none">
        ...
        <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
            ...
            <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase truncate tracking-wide">Perbarui Data Siswa</h3>
                <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs md:text-sm text-white/85 font-medium">
                    <div class="flex items-center gap-2">
                        ...
                        <span>Mengubah Entri: <span class="text-secondary font-bold">{{ $student->name }}</span></span>
                    </div>
                </div>
            </div>
        </div>
...
<div class="max-w-4xl mx-auto mt-6 space-y-6">
    ...
    <form method="POST" action="{{ route('admin.students.update', $student->id) }}" id="edit-student-form" class="space-y-6 px-4 sm:px-0">
        @csrf
        @method('PUT')

        <div class="space-y-3">
            <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Perubahan Instansional Data Siswa</h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2 md:col-span-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $student->name) }}" required class="w-full pl-11 pr-4 py-3.5 bg-slate-50/50 border @error('name') border-danger @else border-slate-200 focus:border-primary @enderror rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all shadow-sm" placeholder="Masukkan nama lengkap">
                        </div>
                        @error('name')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="nisn" id="nisn" value="{{ old('nisn', $student->nisn) }}" required class="w-full pl-11 pr-4 py-3.5 bg-slate-50/50 border @error('nisn') border-danger @else border-slate-200 focus:border-primary @enderror rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all shadow-sm font-mono tracking-wide" placeholder="Contoh: 0071234567">
                        </div>
                        @error('nisn')
                            <p class="text-xs font-bold text-danger mt-1.5 uppercase tracking-wide flex items-center gap-1.5 pl-1"><i class="fa-solid fa-circle-exclamation text-[10px]"></i> {{ $message }}</p>
                        @enderror
