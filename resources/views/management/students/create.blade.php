...
@section('title', 'Tambah Siswa Baru - e-Rapor')
    ...
    <div class="bg-primary text-white pt-10 pb-20 px-6 md:px-8 relative overflow-hidden select-none">
        ...
        <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
            ...
            <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase truncate tracking-wide">Tambah Siswa Baru</h3>
                <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs md:text-sm text-white/85 font-medium">
                    <div class="flex items-center gap-2">
                        <i class="fa-solid fa-users-gear text-base text-secondary opacity-95"></i>
                        <span>Panel Manajemen Kesiswaan</span>
                    </div>
                    <div class="flex items-center gap-2">
...
<div class="max-w-4xl mx-auto mt-6 space-y-6">
    <div class="-mt-16 mb-8 relative z-20 px-4 sm:px-0">
        <a href="{{ route('admin.students.index') }}"
           class="flex sm:inline-flex items-center gap-3.5 bg-white rounded-2xl shadow-md border border-slate-100 p-3 hover:bg-slate-50 transition-colors group w-full sm:w-auto">
            ...
            <div class="flex flex-col justify-center min-w-0 pr-2 text-left">
    ...
    <form method="POST" action="{{ route('admin.students.store') }}" id="create-student-form" class="space-y-6 px-4 sm:px-0">
        @csrf

        <div class="space-y-3">
            <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Formulir Pendaftaran Siswa</h4>
            <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 md:p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                    <div class="space-y-2 md:col-span-2">
                        <label for="name" class="block text-xs font-black text-slate-500 uppercase tracking-wider pl-1">Nama Lengkap Siswa</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                <i class="fa-solid fa-user text-sm"></i>
                            </div>
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full pl-11 pr-4 py-3.5 bg-slate-50/50 border @error('name') border-danger @else border-slate-200 focus:border-primary @enderror rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all shadow-sm" placeholder="Masukkan nama lengkap siswa">
                        </div>
                        @error('name')
                            <p class="text-xs font-bold text-danger mt-1.5 uppercase tracking-wide flex items-center gap-1.5 pl-1"><i class="fa-solid fa-circle-exclamation text-[10px]"></i> {{ $message }}</p>
                        @enderror
                    </div>

                    <div class="space-y-2">
                        <label for="nisn" class="block text-xs font-black text-slate-500 uppercase tracking-wider pl-1">NISN (Nomor Induk Siswa Nasional)</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                <i class="fa-solid fa-id-card text-sm"></i>
                            </div>
                            <input type="text" name="nisn" id="nisn" value="{{ old('nisn') }}" required class="w-full pl-11 pr-4 py-3.5 bg-slate-50/50 border @error('nisn') border-danger @else border-slate-200 focus:border-primary @enderror rounded-xl text-sm font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white transition-all shadow-sm font-mono tracking-wide" placeholder="Contoh: 0071234567">
                        </div>
                        @error('nisn')
                            <p class="text-xs font-bold text-danger mt-1.5 uppercase tracking-wide flex items-center gap-1.5 pl-1"><i class="fa-solid fa-circle-exclamation text-[10px]"></i> {{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-2">
                        <label for="classroom_id" class="block text-xs font-black text-slate-500 uppercase tracking-wider pl-1">Kelas Siswa</label>
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                <i class="fa-solid fa-chalkboard-user text-sm"></i>
                            </div>
                            <select name="classroom_id" id="classroom_id" required class="w-full pl-11 pr-10 py-3.5 bg-slate-50/50 border @error('classroom_id') border-danger @else border-slate-200 focus:border-primary @enderror rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:bg-white transition-all shadow-sm appearance-none cursor-pointer">
        ...
        <div class="bg-blue-50 border-l-4 border-primary text-slate-900 rounded-2xl p-4 shadow-sm flex items-center gap-4 select-none">
            ...
            <div class="flex-1">
                <h5 class="font-extrabold text-xs uppercase tracking-wide text-primary">Integritas Data Kesiswaan</h5>
                <p class="text-xs text-slate-600 mt-0.5 leading-normal">Data NISN siswa bersifat unik dalam lingkup nasional. Pastikan NISN yang diinput telah sesuai dengan data resmi Dapodik kemendikbud demi kelancaran integrasi e-Rapor.</p>
            </div>
        </div>

        <div class="hidden md:flex items-center justify-end gap-4 pt-2">
            <a href="{{ route('admin.students.index') }}" class="px-5 py-3 rounded-xl border border-slate-200 text-xs font-black text-slate-400 uppercase tracking-wider hover:bg-slate-50 transition-all text-center">
                Batal
            </a>
...
<div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-100 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] px-4 py-3 pb-safe">
    <div class="flex items-center gap-3 max-w-lg mx-auto">
        ...
        <button type="submit" form="create-student-form"
                class="flex-[2] flex items-center justify-center gap-2 py-3 rounded-2xl bg-gradient-to-br from-primary to-blue-800 text-white font-black text-xs uppercase tracking-wider shadow-lg shadow-primary/30 hover:shadow-primary/50 transition-all border-b-2 border-blue-900 cursor-pointer">
