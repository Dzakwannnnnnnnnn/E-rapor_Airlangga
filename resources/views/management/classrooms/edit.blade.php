...
@section('title', 'Edit Kelas - ' . $classroom->name . ' - e-Rapor')
        ...
        <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
            ...
            <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                <div class="inline-flex items-center gap-1.5 bg-secondary text-[10px] text-slate-950 font-bold px-2.5 py-0.5 rounded-full mb-2.5 uppercase tracking-wider shadow-sm">
                    <i class="fa-solid fa-layer-group text-[9px]"></i> ID Kelas: #{{ $classroom->id }}
                </div>
                <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase truncate tracking-wide">Edit Kelas</h3>
                ...
                <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs md:text-sm text-white/85 font-medium">
                    ...
                    <div class="flex items-center gap-2">
                        <i class="fa-solid fa-users text-base text-secondary opacity-95"></i>
                        <span>Kapasitas Terdata: <strong class="text-white">{{ $classroom->students_count ?? 0 }} Siswa</strong></span>
                    </div>
                </div>
            </div>
        </div>
...
<div class="max-w-4xl mx-auto mt-6 space-y-6">
    ...
    <form id="edit-classroom-form" method="POST" action="{{ route('admin.classrooms.update', $classroom->id) }}" class="space-y-6 px-4 sm:px-0">
        @csrf
        @method('PUT')

        <div class="space-y-3">
            <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Perbarui Informasi Rombongan Belajar</h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="major" id="major" value="{{ old('major', $classroom->major) }}" required
        ...
        <div class="bg-blue-50 border-l-4 border-primary text-slate-900 rounded-2xl p-4 shadow-sm flex items-center gap-4 select-none">
            ...
            <div class="flex-1">
                <h5 class="font-extrabold text-xs uppercase tracking-wide text-primary">Catatan Relasi Struktural</h5>
                <p class="text-xs text-slate-600 mt-0.5 leading-normal">Mengubah nama ruang kelas atau kompetensi keahlian akan langsung berdampak pada <span class="font-bold text-slate-800">identitas administratif</span> data siswa terdaftar, riwayat cetak rapor, dan penugasan wali kelas terkait.</p>
            </div>
        </div>
...
<div class="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-slate-100 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] px-4 py-3 pb-safe">
    <div class="flex items-center gap-3 max-w-lg mx-auto">
        ...
        <button type="submit" form="edit-classroom-form"
