...
@section('title', 'Manajemen Mata Pelajaran - e-Rapor')
        ...
        <div class="max-w-6xl mx-auto flex flex-col md:flex-row items-center md:items-start justify-between gap-6 relative z-10">
            <div class="flex flex-col md:flex-row items-center md:items-start gap-6">
                ...
                <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                    <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase tracking-wide">Manajemen Mata Pelajaran</h3>
                    <div class="mt-3 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-1.5 text-xs md:text-sm text-white/85 font-medium">
                        <div class="flex items-center gap-2">
                            ...
                            <span>Kelola Komponen Pembelajaran & Kurikulum</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="shrink-0 mt-2 md:mt-4">
                <a href="{{ route('admin.subjects.create') }}" class="inline-flex items-center gap-2 bg-secondary text-primary hover:bg-white font-black text-sm px-6 py-3 rounded-full transition-all shadow-lg active:scale-95">
...
<div class="max-w-6xl mx-auto mt-6 space-y-6">
    ...
    <div class="max-w-xl mx-auto -mt-16 mb-6 relative z-20 px-4">
        <div class="bg-white rounded-2xl shadow-md border border-slate-100 p-5 flex items-center gap-4">
            ...
            <div>
                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Mata Pelajaran</span>
                <span class="text-xl font-black text-slate-800 leading-none mt-1 block">
                    {{ $subjects->total() }} <span class="text-sm font-bold text-slate-500">Komponen</span>
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="px-4 md:px-0 pb-12">
        ...
        <div class="space-y-4">
            <div class="flex items-center gap-2 mb-2">
                <i class="fa-solid fa-book-bookmark text-yellow-600"></i>
                <h4 class="font-bold text-slate-800 text-sm uppercase tracking-wide">Daftar Komponen Pembelajaran</h4>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @forelse ($subjects as $subject)
                    @php
                        $isAcademic = $subject->type === 'academic';
                        $typeName = $isAcademic ? 'Akademik' : 'Ekstrakurikuler';
                    @endphp
                    ...
                    <div class="relative bg-white rounded-xl border border-slate-100 p-4 sm:p-5 flex flex-col justify-between gap-4 hover:shadow-md hover:border-slate-200 transition-all duration-300 group">
                        ...
                        <div class="hidden sm:flex absolute top-5 right-5 items-center gap-1.5 {{ $isAcademic ? 'bg-blue-50 text-blue-700 border-blue-200/60' : 'bg-purple-50 text-purple-700 border-purple-200/60' }} border px-2.5 py-0.5 rounded-full text-[10px] font-black tracking-wide shadow-sm uppercase">
                            <span class="w-1.5 h-1.5 rounded-full {{ $isAcademic ? 'bg-blue-500' : 'bg-purple-500' }}"></span>
                            {{ $typeName }}
                        </div>

                        <div class="flex items-start gap-3 sm:gap-4">
                            <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-amber-50 text-amber-700 flex items-center justify-center font-black text-sm sm:text-base shrink-0 border border-amber-100/50 tracking-wide">
                                {{ strtoupper(substr($subject->name, 0, 2)) }}
                            </div>

                            <div class="flex-1 min-w-0">
                                <div class="flex flex-col items-start gap-1">
                                    <span class="font-bold text-slate-800 text-sm sm:text-base block truncate w-full group-hover:text-primary transition-colors uppercase tracking-wide">
                                        {{ $subject->name }}
                                    </span>

                                    <span class="sm:hidden inline-flex items-center gap-1 {{ $isAcademic ? 'bg-blue-50 text-blue-700 border-blue-200/60' : 'bg-purple-50 text-purple-700 border-purple-200/60' }} border px-2 py-0.5 rounded-full text-[9px] font-black tracking-wide uppercase">
                                        <span class="w-1 h-1 rounded-full {{ $isAcademic ? 'bg-blue-500' : 'bg-purple-500' }}"></span>
                                        {{ $typeName }}
                                    </span>
                                </div>

                                <span class="text-[11px] text-slate-400 block mt-1 font-medium">
                                    <i class="fa-regular fa-calendar-minus mr-1"></i> Terdaftar: {{ $subject->created_at ? $subject->created_at->format('d M Y') : '-' }}
                                </span>
                            </div>
                        </div>

                        <div class="grid grid-cols-2 gap-2 pt-3 border-t border-dashed border-slate-100 mt-auto w-full">
                            <a href="{{ route('admin.subjects.edit', $subject->id) }}" class="inline-flex justify-center items-center gap-1.5 px-2 py-2 bg-slate-50 text-slate-600 hover:text-amber-600 hover:bg-amber-50/50 rounded-lg transition-colors text-xs font-bold border border-slate-100" title="Edit Data">
                            ...
                            <form id="delete-subject-{{ $subject->id }}"
                                  ...
                                  data-confirm="Komponen mata pelajaran &quot;{{ $subject->name }}&quot; akan dihapus secara permanen dari sistem kurikulum."
                ...
                @empty
                    <div class="md:col-span-2 bg-white rounded-xl shadow-sm border border-slate-100 p-12 text-center flex flex-col items-center">
                        ...
                        <p class="text-sm font-bold text-slate-600">Belum ada data Mata Pelajaran</p>
                        @if($search)
