...
@section('title', 'Tambah Mata Pelajaran Baru - e-Rapor')
    ...
    <div class="bg-primary text-white pt-10 pb-20 px-6 md:px-8 relative overflow-hidden select-none">
        ...
        <div class="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 relative z-10">
            ...
            <div class="flex-1 text-center md:text-left min-w-0 mt-1">
                <h3 class="font-extrabold text-xl md:text-3xl leading-tight uppercase truncate tracking-wide">Tambah Mata Pelajaran</h3>
        ...
        <a href="{{ route('admin.subjects.index') }}"
           class="flex sm:inline-flex items-center gap-3.5 bg-white rounded-2xl shadow-md border border-slate-100 p-3 hover:bg-slate-50 transition-colors group w-full sm:w-auto">
            ...
            <div class="flex flex-col justify-center min-w-0 pr-2">
                ...
                <span class="text-xs font-black text-slate-800 uppercase leading-none block truncate">Kembali ke Daftar Mata Pelajaran</span>
            </div>
        </a>
    </div>

    <form id="create-subject-form" method="POST" action="{{ route('admin.subjects.store') }}" class="space-y-6 px-4 sm:px-0">
        @csrf

        <div class="space-y-3">
            <h4 class="text-xs font-black text-slate-400 uppercase tracking-widest pl-1">Formulir Komponen Mata Pelajaran</h4>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <select name="type" id="type" required
                                ...
                                <option value="" disabled {{ old('type') ? '' : 'selected' }}>Pilih Tipe Pembelajaran</option>
        ...
        <div class="bg-blue-50 border-l-4 border-primary text-slate-900 rounded-2xl p-4 shadow-sm flex items-center gap-4 select-none mx-4 sm:mx-0">
            ...
            <div class="flex-1">
                <h5 class="font-extrabold text-xs uppercase tracking-wide text-primary">Validasi Komponen Nilai</h5>
                <p class="text-xs text-slate-600 mt-0.5 leading-normal">Penambahan komponen mata pelajaran baru akan <span class="font-bold text-slate-800">mempengaruhi pemetaan struktur nilai</span> pada setiap kelas. Pastikan data tipe akademik atau ekstrakurikuler telah sesuai dengan kurikulum satuan pendidikan.</p>
            </div>
        </div>
